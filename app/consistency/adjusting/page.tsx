...
function ConsistencyAdjustingContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const consistency = searchParams.get("consistency") ?? "55";
  ...
  useEffect(() => {
    ...
    const t4 = setTimeout(() => router.replace("/consistency/preview?consistency=" + consistency), 3000);
    return () => {
      clearTimeout(t1);
      clearTimeout(t2);
      clearTimeout(t3);
...
export default function ConsistencyAdjustingPage() {
  return (
    <Suspense>
      <ConsistencyAdjustingContent />
    </Suspense>
  );
}
